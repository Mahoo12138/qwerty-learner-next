...
func (api *BaseApi) Captcha(c *gin.Context) {
	// 判断验证码是否开启
	...
	// 字符,公式,验证码配置
	// 生成默认数字的driver
	driver := base64Captcha.NewDriverDigit(global.QL_CONFIG.Captcha.ImgHeight, global.QL_CONFIG.Captcha.ImgWidth, global.QL_CONFIG.Captcha.KeyLong, 0.7, 80)
	// cp := base64Captcha.NewCaptcha(driver, store.UseWithCtx(c))   // v8下使用redis
	cp := base64Captcha.NewCaptcha(driver, store)
	id, b64s, _, err := cp.Generate()
	if err != nil {
		global.QL_LOG.Error("验证码获取失败!", zap.Error(err))
	...
	response.OkWithDetailed(systemRes.CaptchaResponse{
		CaptchaId:     id,
		PicPath:       b64s,
		CaptchaLength: global.QL_CONFIG.Captcha.KeyLong,
		OpenCaptcha:   oc,
	}, "验证码获取成功", c)
}
